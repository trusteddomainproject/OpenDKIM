...
**
**  $Id: opendkim.c,v 1.214 2010/09/21 21:01:03 cm-msk Exp $
*/

#ifndef lint
static char opendkim_c_id[] = "@(#)$Id: opendkim.c,v 1.214 2010/09/21 21:01:03 cm-msk Exp $";
#endif /* !lint */
...
dkimf_xs_requestsig(lua_State *l)
{
	...
	if (lua_gettop(l) == 0 && lua_gettop(l) > 3)
	{
		...
		lua_error(l);
	         ...
	         (lua_gettop(l) > 1 && !lua_isstring(l, 2)) ||
	         (lua_gettop(l) > 2 && !lua_isstring(l, 3)))
	{
	...
	else if (dkimf_add_signrequest(dfc, NULL, NULL, (char *) ident) != 0)
	{
		if (conf->conf_dolog)
...
dkimf_reportaddr(struct dkimf_config *conf)
{
	...
	if (conf->conf_reportaddr != NULL)
	{
		...
		strlcpy(env, conf->conf_reportaddr, sizeof reportaddr);
		...
		if (dfc->mctx_addheader)
		{
			...
			*/
			...
			if (dfc->mctx_status != DKIMF_STATUS_UNKNOWN)
			{
				...
				strlcat((char *) header, "dkim=",
				        sizeof header);
				strlcat((char *) header, authresult,
				        sizeof header);
