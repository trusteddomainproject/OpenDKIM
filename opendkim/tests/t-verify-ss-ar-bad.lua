...
mt.echo("*** simple/simple verifying with oversized Authentication-Results")
...
if mt.header(conn, "Authentication-Results", ar) ~= nil then
	error("mt.header(Authentication-Results) failed")
end
if mt.getreply(conn) ~= SMFIR_CONTINUE then
	error("mt.header(Authentication-Results) unexpected reply")
end
