-- $Id: t-sign-ss-ltag.lua,v 1.19 2010/09/24 21:40:31 cm-msk Exp $
...
mt.echo("*** simple/simple signing test with l= tag")

-- setup
sock = "unix:" .. mt.getcwd() .. "/t-sign-ss-ltag.sock"
binpath = mt.getcwd() .. "/.."
...
mt.startfilter(binpath .. "/opendkim", "-x", "t-sign-ss-ltag.conf", "-p", sock)
...
mt.macro(conn, SMFIC_MAIL, "i", "t-sign-ss-ltag")
...
if string.find(sig, " l=", 1, true) == nil then
	error("signature is missing l= value")
end
