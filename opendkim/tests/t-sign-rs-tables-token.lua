-- $Id: t-sign-rs-tables-token.lua,v 1.4 2010/09/06 06:41:47 grooverdan Exp $

-- Copyright (c) 2009, 2010, The OpenDKIM Project.  All rights reserved.

-- relaxed/simple signing test using KeyTable/SigningTable with domain token
-- 
...
mt.macro(conn, SMFIC_MAIL, "i", "t-sign-rs-tables-token")
...
if mt.header(conn, "From", "user2@example.com") ~= nil then
...
-- send envelope macros and sender data
-- mt.helo() is called implicitly
mt.macro(conn, SMFIC_MAIL, "i", "t-sign-rs-tables-token-2")
if mt.mailfrom(conn, "user@example.com") ~= nil then
	error "mt.mailfrom() failed"
end
if mt.getreply(conn) ~= SMFIR_CONTINUE then
...
-- send headers
-- mt.rcptto() is called implicitly
if mt.header(conn, "From", "user3@example.com") ~= nil then
	error "mt.header(From) failed"
end
if mt.getreply(conn) ~= SMFIR_CONTINUE then
	error "mt.header(From) unexpected reply"
...
if string.find(sig, "i=signer@foo.example.com", 1, true) == nil then
	error "signature has wrong i= value"
end
