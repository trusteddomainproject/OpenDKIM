...
-- simple/simple signing test with "x=" tag
-- 
-- Confirms that a signature is added with the correct contents.

mt.echo("*** simple/simple signing test with 'x=' tag")

-- setup
if TESTSOCKET ~= nil then
	sock = TESTSOCKET
else
	sock = "unix:" .. mt.getcwd() .. "/t-sign-ss-x.sock"
end
...
mt.startfilter(binpath .. "/opendkim", "-x", "t-sign-ss-x.conf", "-p", sock)
...
mt.macro(conn, SMFIC_MAIL, "i", "t-sign-ss-x")
...
if string.find(sig, "x=", 1, true) == nil then
	error("signature has no x= value")
end
