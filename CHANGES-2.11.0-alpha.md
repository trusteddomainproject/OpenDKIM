...
- **Lua detection rewrite**: Overhauled `configure.ac` Lua detection to use pkg-config where available, with manual fallback. Supports Lua 5.1-5.5, respects `LUA_CFLAGS`/`LUA_LIBS` environment variables, drops the stale `lua5.1` pkg-config name. (#264, #266, #327, issues #111)
...
- **`--with-milter` explicit path**: When an explicit path was given, configure still ran a link test that could override it. (#336, issue #85)
