...
- **Malformed `DKIM-Signature` suppressed `Authentication-Results`**: When a message arrived with a syntactically invalid `DKIM-Signature` header and `OnBadSignature` was set to `accept`, `mlfi_eoh()` returned early before `mlfi_eom()` ran, so `AlwaysAddARHeader` and `SoftwareHeader` never fired. The message was accepted with no `Authentication-Results` header, indistinguishable from mail never processed by OpenDKIM. RFC 8601 section 2.7.1 defines `dkim=permerror` for this case; OpenDKIM now emits it. `OnBadSignature reject/tempfail/discard` behaviour is unchanged. (#403, issue #194)

- **CVE audit**: All CVEs on record against OpenDKIM (CVE-2020-35766, CVE-2022-48521) were confirmed fixed in the develop branch. Each has a persistent entry in the SECURITY directory.
- **CVE-2020-35766**: The test suite wrote generated key material to the fixed path `/tmp/testkeys`, enabling a local symlink attack. Tests now write to `./testkeys` in the build tree. This vulnerability affected only users who built from source and ran `make check`; consumers of pre-packaged OS distributions are not affected, as distribution packages do not ship the test suite. (#260, #288)
- **CVE-2022-48521**: `Authentication-Results` headers were removed in forward order; removing one shifted the index of the next, allowing a crafted second forged header to survive. Fixed to remove in reverse order. The same class of off-by-one was present in the Lua `del_header()` helper and in the `KeepAuthResults` deletion path; both were corrected. (#287)
