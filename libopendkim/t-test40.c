...
#ifndef lint
static char t_test40_c_id[] = "@(#)$Id: t-test40.c,v 1.4 2009/11/11 19:40:00 cm-msk Exp $";
#endif /* !lint */
...
main(int argc, char **argv)
{
	...
	{
		printf("*** relaxed/simple rsa-sha256 verifying with no leading spaces SKIPPED\n");
		dkim_close(lib);
		return 0;
	}

	printf("*** relaxed/simple rsa-sha256 verifying with no leading spaces\n");
	...
	status = dkim_header(dkim, HEADER07NOLEADSP, strlen(HEADER07NOLEADSP));
	assert(status == DKIM_STAT_OK);
