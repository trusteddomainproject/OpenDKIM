...
#ifndef lint
static char t_test45_c_id[] = "@(#)$Id: t-test45.c,v 1.1 2009/07/16 19:12:04 cm-msk Exp $";
#endif /* !lint */
...
prescreen(DKIM *dkim, DKIM_SIGINFO **sigs, int nsigs)
{
	...
	/*
	**  Arrange to skip the one that's simple/simple since we know
	**  it's bad for this test.
	*/

	for (c = 0; c < nsigs; c++)
	{
		ls = dkim_sig_getcontext(sigs[c]);
		if (strcasecmp(ls->ls_canon, "simple/simple") == 0)
...
main(int argc, char **argv)
{
	...
	printf("*** relaxed/simple rsa-sha256 signature selection SKIPPED\n");
...
# endif /* TEST_KEEP_FILES */
	...
	printf("*** relaxed/simple rsa-sha256 signature selection\n");
