...
#ifndef lint
static char t_test33_c_id[] = "@(#)$Id: t-test33.c,v 1.2 2009/12/08 19:14:27 cm-msk Exp $";
#endif /* !lint */
...
main(int argc, char **argv)
{
	...
	status = dkim_header(dkim, HEADER05 " ", strlen(HEADER05) + 1);
	assert(status == DKIM_STAT_OK);

	status = dkim_header(dkim, HEADER06 "  ", strlen(HEADER06) + 2);
	assert(status == DKIM_STAT_OK);

	status = dkim_header(dkim, HEADER07 " ", strlen(HEADER07) + 1);
	assert(status == DKIM_STAT_OK);

	status = dkim_header(dkim, HEADER08 "  ", strlen(HEADER08) + 2);
	assert(status == DKIM_STAT_OK);

	status = dkim_header(dkim, HEADER09 " ", strlen(HEADER09) + 1);
	assert(status == DKIM_STAT_OK);
