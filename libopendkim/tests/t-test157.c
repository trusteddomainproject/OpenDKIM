...
main(int argc, char **argv)
{
	...
	{
		printf("*** conditional signature verifying (altered) SKIPPED\n");
		dkim_close(lib);
		return 0;
	}

	printf("*** conditional signature verifying (altered)\n");
	...
	*/
	...
	for (c = 0; c < nsigs; c++)
	{
		if (strcasecmp(dkim_sig_getdomain(sigs[c]), DOMAIN2) == 0)
		{					/* sendmail.com */
		...
		{					/* example.com */
			assert((dkim_sig_getflags(sigs[c]) & DKIM_SIGFLAG_PASSED) != 0);
			assert(dkim_sig_getbh(sigs[c]) == DKIM_SIGBH_MISMATCH);
			assert(dkim_sig_geterror(sigs[c]) == DKIM_SIGERROR_BADSIG);
		}
	}
