...
#ifndef lint
static char t_test138_c_id[] = "@(#)$Id: t-test04.c,v 1.2 2009/12/08 19:14:27 cm-msk Exp $";
#endif /* !lint */
...
main(int argc, char **argv)
{
	...
	assert(sig != NULL);
	nsigs = SIGARRAY;
	status = dkim_sig_getsignedhdrs(dkim, sig, (u_char *) signedhdrs,
	                                DKIM_MAXHEADER + 1, &nsigs);
	assert(status == DKIM_STAT_OK);
	assert(nsigs == 8);
	assert(strcmp(signedhdrs[0], HEADER04) == 0);
	assert(strcmp(signedhdrs[1], HEADER03) == 0);
	assert(strcmp(signedhdrs[2], HEADER02) == 0);
	assert(strcmp(signedhdrs[3], HEADER05) == 0);
	assert(strcmp(signedhdrs[4], HEADER06) == 0);
	assert(strcmp(signedhdrs[5], HEADER07) == 0);
	assert(strcmp(signedhdrs[6], HEADER08) == 0);
	assert(strcmp(signedhdrs[7], HEADER09) == 0);
