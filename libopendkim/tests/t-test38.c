...
#ifndef lint
static char t_test38_c_id[] = "@(#)$Id: t-test38.c,v 1.1 2009/11/28 19:02:37 cm-msk Exp $";
#endif /* !lint */
...
main(int argc, char **argv)
{
	...
	{
		printf("*** relaxed/simple rsa-sha256 verifying with header unwrapping SKIPPED\n");
		dkim_close(lib);
		return 0;
	}

	printf("*** relaxed/simple rsa-sha256 verifying with header unwrapping\n");
	...
	status = dkim_header(dkim, HEADER04UNWRAP, strlen(HEADER04UNWRAP));
	assert(status == DKIM_STAT_OK);
