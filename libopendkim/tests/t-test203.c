...
final(DKIM *dkim, DKIM_SIGINFO **sigs, int nsigs)
{
	...
	/* verify that all three signatures are okay */
	...
	status = dkim_sig_process(dkim, sigs[2]);
	assert(status == DKIM_STAT_OK);
	flags = dkim_sig_getflags(sigs[2]);
	assert((flags & DKIM_SIGFLAG_PROCESSED) != 0);
	assert((flags & DKIM_SIGFLAG_IGNORE) == 0);
	assert((flags & DKIM_SIGFLAG_PASSED) != 0);
	assert(dkim_sig_getbh(sigs[2]) == DKIM_SIGBH_MATCH);
