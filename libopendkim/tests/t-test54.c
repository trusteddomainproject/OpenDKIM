...
#ifndef lint
static char t_test54_c_id[] = "@(#)$Id: t-test54.c,v 1.2 2009/12/08 19:14:27 cm-msk Exp $";
#endif /* !lint */
...
main(int argc, char **argv)
{
	...
	{
		printf("*** relaxed/simple rsa-sha256 signing with header selection SKIPPED\n");
		dkim_close(lib);
		return 0;
	}

	printf("*** relaxed/simple rsa-sha256 signing with header selection\n");
...
#define	XHEADER2	"X-ExtraHeader-2: value2"

	status = dkim_header(dkim, XHEADER1, strlen(XHEADER1));
	assert(status == DKIM_STAT_OK);

	status = dkim_header(dkim, XHEADER2, strlen(XHEADER2));
	assert(status == DKIM_STAT_OK);
